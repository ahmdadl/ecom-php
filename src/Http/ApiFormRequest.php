...
//    /**
    ...
    {
        $requestKeys = array_unique(array_map(fn($input) => str_contains($input, '.') ? explode('.', $input)[0] : $input, $inputs));
        ...
        collect($requestInputsDotNotation)->each(function ($value, $key) use ($inputs, &$requestInputs) {
            ...
            $filteredKeyArray = array_filter($keyArray, fn($item) => preg_match_all('!\d+!', $item) === 1);
